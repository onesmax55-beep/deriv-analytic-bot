...
async function initializeServices() {
  ...
  liveAlertIntegration = new LiveAlertIntegration({ marketScanner, alertEngine });
  ...
  liveAlertIntegration.start();
}

async function startLiveData() {
  try {
    await connectionManager.connect();
    await marketScanner.start(await scannerConfiguration.getMarkets());
    console.log('[main] Deriv connection and live scanner started');
  } catch (error) {
    console.error('[main] Live data startup failed; application will remain available for retry:', error);
  }
}
...
async function shutdownApplication() {
  ...
  try { liveAlertIntegration?.dispose?.(); } catch (_) {}
  try { alertUnsubscribe?.(); } catch (_) {}
  try { alertEngine?.dispose?.(); } catch (_) {}
  try { await marketScanner?.stop?.(); } catch (_) {}
  try { marketScanner?.dispose?.(); } catch (_) {}
  try { await connectionManager?.disconnect?.(); } catch (_) {}
...
module.exports = { createMainWindow, initializeServices, startLiveData, shutdownApplication, getServices: () => ({ database, analyticsEngine, replayEngine, playbackController, sessionRepository, alertHistoryRepository, settingsManager, connectionManager, marketScanner, scannerConfiguration, alertEngine, liveAlertIntegration, notificationService }) };
