...
class ConnectionManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.reconnectManager = new ReconnectManager({ maxRetries: options.maxRetries || 10 });
    this.messageRouter = new MessageRouter();
    this.heartbeat = new Heartbeat({ interval: options.heartbeatInterval || 30000 });
  ...
  setupEventHandlers() {
    ...
    this.subscriptionManager.on('subscription-confirmed', (data) => this.emit('subscription-active', data));
    this.subscriptionManager.on('subscription-failed', (data) => this.emit('subscription-error', data));
    this.heartbeat.on('stale', () => this.onHeartbeatStale());
    this.reconnectManager.on('reconnect-attempt', (data) => this.emit('reconnect-attempt', data));
  ...
  async connect() {
    if (this.state === CONNECTION_STATE.CONNECTED || this.state === CONNECTION_STATE.CONNECTING) return;
    this.isDisconnecting = false;
  ...
  async subscribe(symbol) {
    if (this.state !== CONNECTION_STATE.CONNECTED) throw new Error('Not connected to Deriv');
    try {
      if (!this.tickStreams.has(symbol)) this.tickStreams.set(symbol, new TickStream(symbol));
      ...
      this.subscriptionManager.confirm(reqId, subscriptionId);
      this.emit('subscribed', { symbol, subscriptionId });
      return response;
    } catch (error) {
    ...
    try {
      ...
      for (const subscription of subscriptions) {
        if (subscription.subscriptionId) {
          await this.client.request('forget', { forget: subscription.subscriptionId });
        }
  ...
  sendPing() { this.client.send({ ping: 1 }); }
  ...
  onClientDisconnected() {
    ...
    this.reconnectManager.attemptReconnect(async () => { await this.client.connect(); });
  }

  onClientError(error) { this.emit('error', error); }
  ...
  onTickReceived(message) {
    ...
    if (!resolvedSymbol || !this.tickStreams.has(resolvedSymbol)) return;
    const tick = { symbol: resolvedSymbol, quote, time: epoch };
    this.tickStreams.get(resolvedSymbol).addTick(tick);
    this.emit('tick', tick);
  }

  onErrorReceived(message) { this.emit('deriv-error', message.error); }
  onPongReceived() { this.heartbeat.pong(); }
  ...
  onReconnected() {
    ...
    subscriptions.forEach((params) => this.client.send({ subscribe: 1, ...params }));
  ...
  getTickStream(symbol) { return this.tickStreams.get(symbol); }
  getTickStreams() { return Array.from(this.tickStreams.values()); }
}
